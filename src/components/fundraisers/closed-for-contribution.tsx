...
interface ClosedForContributionProps {
  ...
  sharePath?: string;
  /** Host-written closing note. Replaces the generated body when present. */
  message?: string;
}
...
export function ClosedForContribution({
  ...
  const hostMessage = message?.trim();
  const body = hostMessage
    ? hostMessage
    : [raisedSentence, impactSentence, t('closedNote')]
        .filter(Boolean)
        .join(' ');
