...
export const StripeCardForm = forwardRef<
  StripeCardFormHandle,
  StripeCardFormProps
>(function StripeCardForm({ onReadyChange }, ref) {
  ...
  const [billingZipCodeError, setBillingZipCodeError] = useState<string | null>(
    null
  );
  const [billingCountry, setBillingCountry] = useState('');
  const [billingCountryError, setBillingCountryError] = useState<string | null>(
    null
  );
  ...
  useImperativeHandle(ref, () => ({
    async createPaymentMethod({ email, donorAddress }) {
      ...
      if (!cardNumberElement) return { error: t('errors.cardElementNotFound') };
