...
export const StripeSepaForm = forwardRef<
  StripeSepaFormHandle,
  StripeSepaFormProps
>(function StripeSepaForm({ onReadyChange }, ref) {
  ...
  useEffect(() => {
    const hasErrors = !!ibanError || !!nameError || !!mandateError;
    const isComplete =
      ibanComplete && accountHolderName.trim().length > 0 && mandateAccepted;
    onReadyChange?.(!hasErrors && isComplete);
  }, [
    ibanError,
    nameError,
    mandateError,
  ...
  return (
    <div className='space-y-4'>
      ...
      <FormField label={t('accountHolderName')} error={nameError ?? undefined}>
        <Input
