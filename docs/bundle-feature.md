...
- `bundleToAllocations(bundle, workspace, getProject?): Array<{ project_id; percentage }>` — applies the legacy 25% floor on the support project (see Decision #10). For 5-project bundles → `[28, 18, 18, 18, 18]`. The arithmetic is inlined; an earlier draft of this step pointed at a separate `getEqualSplit` helper which was removed during implementation since it had no other callers. When `getProject` is passed, non-donatable projects are dropped from the allocation and the donatable ones absorb the freed share, then the dropped projects are appended back at `percentage: 0` so the API receives the full bundle list; the donatable projects still sum to 100. Both the create and edit forms pass `getProject` (the "Use this bundle" handler), so both keep non-donatable bundle projects in the payload at 0%. The 0% entries stay hidden in the views and are ignored by the donation flow (both already filter `allowDonations === false`). The form schema's `projectAllocationSchema` allows `percentage` down to `0` for this reason
