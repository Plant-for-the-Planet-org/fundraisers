...
The supplied config has a `meta.workspace` field (`"DE"`) that drives country gating (see Step 4a). The 4 curated project IDs per bundle are stored in `bundle.projects`. The **support project** is _not_ embedded in each bundle — it lives in a sibling `supportProjects` map (see Decision #3 in the table above).
...
- `Bundle` — `{ slug: BundleSlug; label: string; tabs: BundleTabId[]; tagline: string; projectIds: string[] }` (4 curated IDs; support ID is _not_ in this list)
...
- `getWorkspaceForCountry(country: AllowedCountry | undefined): BundleWorkspace | null` — in `bundle-country-mapping.ts`; reads `bundleWorkspace` from the workspace registry (`src/lib/workspaces/registry.ts`). Returns `null` when only Custom should be exposed
...
- `getDonatableBundleProjectIds(bundle, workspace, getProject): string[]` — `getBundleProjectIds` filtered to IDs whose resolved project has `allowDonations === true`. Used by the bundle card and preview modal to hide projects that no longer accept donations, so a bundle may render fewer than 5. Display only — the allocations written on "Use this bundle" still come from `bundleToAllocations`
- `bundleToAllocations(bundle, workspace, getProject?): Array<{ project_id; percentage }>` — applies the legacy 25% floor on the support project (see Decision #10). For 5-project bundles → `[28, 18, 18, 18, 18]`. The arithmetic is inlined; an earlier draft of this step pointed at a separate `getEqualSplit` helper which was removed during implementation since it had no other callers. When `getProject` is passed, non-donatable projects are dropped from the allocation entirely and the donatable ones absorb the freed share, so the donatable projects sum to 100 and the excluded projects are never sent to the API. Both the create and edit forms pass `getProject` (the "Use this bundle" handler). The edit form also filters non-donatable projects out of `fundraiserToFormValues` on load, so they cannot be carried back into the payload on save. The form schema's `projectAllocationSchema` requires `percentage` to be at least `1`, since 0% entries should never reach the API
...
- `bundle-preview-modal.tsx` — opened by clicking a bundle card. Header shows the package icon, bundle label, the orange contextual tag (`STAFF PICKS` / `WONDER` / `RAGE` / `LOVE`), tagline in quoted italics, and an X close button. Body lists the bundle's donatable projects (support + up to 4 curated, resolved via `getDonatableBundleProjectIds(bundle, workspace, getProject)`). Projects with `allowDonations === false` are filtered out, so a bundle may show fewer than 5 rows.
...
Not implemented. Reason: the only ID known to be missing from `getCauseSelectableProjects` is the workspace's support project (it's filtered out because its `purpose` isn't `trees`/`conservation`). `useBundleProjects.getProject` falls back to a synthetic `ProjectData` built from `DEFAULT_NON_EARMARKED_CAUSE_FALLBACK` for that one ID (kept `allowDonations: true` so the support project is never hidden). Any *other* curated ID missing from the selectable list (e.g. a project that stopped accepting donations) falls through to the unknown-project fallback, which is marked `allowDonations: false` so `getDonatableBundleProjectIds` hides it from the card and modal instead of rendering an empty placeholder tile.
...
- `workspace.country` is typed as `AllowedCountry` (`src/lib/types/fundraiser.ts`); the shared `toAllowedCountry` helper from `country-currency.ts` coerces raw values (anything outside `ALLOWED_COUNTRIES` falls back to `'ROW'`, which the workspace registry serves via DE). Custom-only workspaces (ES, CH) still get the per-row country + percentage because `useBundleProjects` runs unconditionally; they just don't render the header (no bundle slug).
...
- **Trimmed `src/lib/utils/project-allocation.ts`** (renamed from `project-selection.ts`)**:** removed `mapProjectToSelectedCause` and `createDefaultCause` (only consumed by the deleted components). `getDefaultCauseId` now reads `defaultCauseId` from the workspace registry (`src/lib/workspaces/registry.ts`) via `getWorkspaceProfile(toAllowedCountry(...))`; the former `resolveCauseCountry` helper was removed. `getDefaultCauseId` and `calculateProjectAllocations` stay — both are core dependencies of the bundle/custom flows.
- **`src/lib/constants/project-selection.ts`:** the surviving exports are `MIN_DEFAULT_CAUSE_PERCENT` (used by `bundleToAllocations` and the custom tab) and `DEFAULT_NON_EARMARKED_CAUSE_FALLBACK` (used by `useBundleProjects`). The per-country cause maps (`DEFAULT_NON_EARMARKED_CAUSE_BY_COUNTRY`, `DEFAULT_NON_EARMARKED_CAUSE_ID`) moved into the workspace registry.
...
- Types in `src/lib/types/project-selection.ts` still in use: `ProjectData`, `SelectedProject`, `ProjectAllocationPreview`, `ProjectPurpose`, `PROJECT_PURPOSES`. (`DefaultCauseIdByCountry` was removed — per-workspace cause ids now live in the workspace registry.)
