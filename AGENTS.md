...
Staff can impersonate a host. The impersonation lives in the browser (`useImpersonationStore`, localStorage), and `platformFetch` adds the `x-switch-user` and `x-user-support-pin` headers from it.

On the server that store is always empty. So any API route of ours that calls ForestCloud on the user's behalf must get the impersonation from the request: the client sends `impersonationHeaders(getActiveImpersonation())`, and the route reads it with `readImpersonation(request.headers)` and passes it on. Without this, the route runs as the staffer, not the host.

When adding a feature, ask: does it show or change data for "the current user"? If so, check it while impersonating.
...
- Put each distinct point on its own line. A point may run to several sentences; keep those together on one line.
- If a point is too long to read comfortably, shorten it or split it into separate points rather than wrapping it.
